...
function translator.init(env)
    ...
    if not schemas and Memory then
        local hao_rev = ReverseLookup and ReverseLookup("hao.base")
        schemas = {
            hao_base = Memory(env.engine, Schema("hao.base")),
            hao_full = Memory(env.engine, Schema("hao.words")),
            hao_trie = core.gen_smart_trie(hao_rev, "hao.smart"),
        }
    elseif not schemas then
        schemas = {}
    end
    if not core.base_mem and schemas then
        core.base_mem = schemas.hao_base
        core.full_mem = schemas.hao_full
        core.word_trie = schemas.hao_trie
    end
