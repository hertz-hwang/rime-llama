...
local processor = rime.make_option_cycler(options,
    'llama_spelling/lua/cycle_key',
    'llama_spelling/lua/switch_key')
...
local function get_tricomment(cand, env)
  ...
  if utf8.len(text) == 1 then
    ...
    return env.engine.context:get_option('llama_spelling.lv1')
      and xform(raw_spelling:gsub('%[(.-),.*%]', '[%1]'))
      or env.engine.context:get_option('llama_spelling.lv2')
      and xform(raw_spelling:gsub('%[(.-,.-),.*%]', '[%1]'))
      or xform(raw_spelling)  -- llama_spelling.lv3 is on
  elseif utf8.len(text) > 1 then
    ...
    if env.engine.context:get_option('llama_spelling.lv1') then
...
local function filter(input, env)
  if env.engine.context:get_option('llama_spelling.off') then
