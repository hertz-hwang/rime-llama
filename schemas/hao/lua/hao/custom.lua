...
function custom.add_smart()
    ...
    function adder:get_word(args, env)
        ...
        for _, code in ipairs(args) do
            ...
            -- 查詢當前單字
            local entries = HaoCore.dict_lookup(env, HaoCore.base_mem, code, index or 1)
    ...
    function adder:peek(args, env)
        ...
        if #chars == 0 or index then
            ...
            -- 顯示最末單字的候選
            local entries = HaoCore.dict_lookup(env, HaoCore.base_mem, codes[#codes], 9)
            local cands = ""
    ...
    function adder:eval(args, env)
        ...
        for i = 1, #chars - 1, 1 do
            ...
            for j = i + 1, #chars, 1 do
                code = code .. codes[j]
                word = word .. chars[j]
                HaoCore.word_trie:update(code, word, weight)
            end
        end
        return ""
    end
...
function custom.del_smart()
    ...
    function deleter:peek(args)
        if #args ~= 0 then
            -- 首個參數爲編碼, 第二個參數爲選重
            local words = HaoCore.word_trie:query(args[1], nil, 9)
            local index
            if #args > 1 then
    ...
    function deleter:eval(args)
        if #args ~= 0 then
            ...
            local words = HaoCore.word_trie:query(code, nil, 9)
            HaoCore.word_trie:delete(code, words[index] or "")
        end
        return ""
    end
...
local function convert_text_to_image(os_name)
    local filename_txt = "/tmp/hao_capture.txt"
    local filename_png = "/tmp/hao_capture.png"

    -- 命令序列
    local cmds = {}
    local copy_cmd = "wl-copy -o <%s"
    if os_name == haoCore.os_types.mac then
        copy_cmd = "impbcopy %s"
    end
...
function custom.capture_embeded()
    -- 判斷系統版本
    local os_name = haoCore.os_name
    if os_name ~= haoCore.os_types.linux and os_name ~= haoCore.os_types.mac then
        return nil
    end
