...
-- options 要与方案保持一致
local options = {
  'leopard_spelling.off',
  'leopard_spelling.lv1',
  'leopard_spelling.lv2',
  'leopard_spelling.lv3'
}
options.default = 4


local processor = rime.make_option_cycler(options,
    'leopard_spelling/lua/cycle_key',
    'leopard_spelling/lua/switch_key')
...
local function get_tricomment(cand, env)
  ...
  if utf8.len(text) == 1 then
    ...
    return env.engine.context:get_option('leopard_spelling.lv1')
      and xform(raw_spelling:gsub('%[(.-),.*%]', '[%1]'))
      or env.engine.context:get_option('leopard_spelling.lv2')
      and xform(raw_spelling:gsub('%[(.-,.-),.*%]', '[%1]'))
      or xform(raw_spelling)  -- leopard_spelling.lv3 is on
  elseif utf8.len(text) > 1 then
    ...
    if env.engine.context:get_option('leopard_spelling.lv1') then
...
local function filter(input, env)
  if env.engine.context:get_option('leopard_spelling.off') then
