...
-- options 要与方案保持一致
local options = {
  'llama_52p_precand.off',
  'llama_52p_precand.brief',
  'llama_52p_precand.detailed'
}
options.default = 2


local processor = rime.make_option_cycler(options,
    'llama_52p_precand/lua/cycle_key',
    'llama_52p_precand/lua/switch_key')
...
local function yield_with_precand(input, context, env)
  ...
  local detailed = context:get_option("llama_52p_precand.detailed")
  -- 这是以候选迭代为基础的，因此要求无空码。
...
local function filter(input, env)
  ...
  if not context:get_option("llama_52p_precand.off")
